# applyOS â€” Marketing Website Brief

## What This Is

This document is the complete brief for the applyOS marketing website. Use it as the prompt for a designer, a web builder (Framer, Webflow), or a UI generation tool (v0.dev, Vercel v0).

applyOS is a personal job search operating system. It scans job boards, scores every role Aâ€“F, generates tailored ATS-optimized CVs, tracks your pipeline, and preps you for interviews â€” all in one structured workflow. It gets smarter the more you use it.

---

## Brand Positioning

**Product name:** applyOS  
**Tagline:** "Your job search, systematized."  
**One-liner:** The end-to-end operating system for serious job seekers.

**Positioning:** applyOS turns a chaotic, manual process into a structured, data-driven workflow. Evaluate more roles, waste less time on poor fits, and walk into every interview overprepared.

**Audience:** Senior ICs, PMs, and managers actively running parallel job tracks â€” especially those juggling high application volume, multiple markets, or complex multi-round processes.

**Tone:**  
Confident, precise, slightly elevated. Not motivational-poster energy â€” more like a Bloomberg terminal crossed with Notion. Speak to professionals who treat their career like the asset it is. No "dream job." No "land your next role." Those are tired.

Copy rules:
- Use active verbs: scan, score, track, generate, detect, prep, draft
- Precision over enthusiasm: "45+ portals" beats "thousands of jobs"
- Avoid hollow adjectives: "powerful," "seamless," "effortless" â€” show capability, don't describe it
...
- **Display / hero:** Inter or Geist â€” 56â€“72px, weight 700, tight tracking (`letter-spacing: -0.02em`)
- **Section headlines:** 36â€“42px, weight 600
- **Body:** 16â€“18px, weight 400, line-height 1.65
- **Data / scores / code:** JetBrains Mono or Geist Mono â€” 12â€“14px, used for numbers, grade badges, market codes, API names
...
- Score badges: letter grades (A, B, C, D, F) as small colored chips â€” green, amber, orange, red respectively
- Status chips: `applied`, `interviewing`, `offer`, `rejected` â€” same palette as the app
...
- Lucide icons sparingly â€” one per feature panel, 20px, no decorative use
...
**Headline (56â€“72px, white, tight tracking):**
```
Your job search,
systematized.
```
...
```
Scan hundreds of roles. Score them Aâ€“F. Generate tailored CVs.
Track your pipeline. Prep for every interview. One system.
```

**CTAs (row, centered):**
- Primary: `Get Started â†’` â€” blue solid button, 16px, medium weight
- Secondary: `See how it works` â€” ghost button, scrolls to Section 3

**Hero visual (below CTAs):**  
A stylized browser-chrome frame containing the pipeline view UI. Show 4â€“5 job cards in a vertical list. Each card displays:
...
**Layout:** Light background (`#F8FAFC`). Two columns on desktop â€” left: text, right: abstract "before" visual. Single column on mobile.
...
applyOS fixes this â€” not with motivation, but with structure.
```

**Right visual:**  
A stylized "chaos" representation â€” a simple text-art grid showing a fragment of a messy spreadsheet: mismatched columns, incomplete rows, duplicate entries, "???" in the status field. Monospace font, `#DC2626` for error-like cells, muted for the rest. Abstract and recognizable, not literal.
...
```
01. Scan & Score  â†’  02. Apply & Track  â†’  03. Prepare & Win
```

Step descriptions (body text, 16px):

**01 â€” Scan & Score**  
Pull roles from 45+ portals via Greenhouse, Ashby, and Lever â€” zero LLM cost for discovery. Every role scored Aâ€“F across structured criteria. Duplicate detection built in so you never evaluate the same posting twice.

**02 â€” Apply & Track**  
Generate ATS-optimized CVs tailored to each JD â€” built from your real experience, not hallucinated. Track every application with canonical status states. Follow-up cadence alerts so nothing goes cold.
...
### Section 4 â€” Features Grid

**Layout:** Light background. 2Ã—3 card grid on desktop, 1-column on mobile. Each card: Lucide icon (top-left), bold headline, 2-sentence description. Thin border, no shadow.
...
Every role graded Aâ€“F before you read a word. Structured evaluation across compensation, fit, role quality, and posting legitimacy â€” your attention goes where it counts.

**3. Tailored CVs, Instantly**  
ATS-optimized CVs built from your actual proof points â€” not fabricated. Output as HTML, PDF via Playwright, or LaTeX/Overleaf-ready, depending on what the application requires.
...
Deep company research before you apply or interview. LinkedIn contact discovery with drafted outreach messages. STAR+R stories surfaced by competency match â€” not generic prep.

**6. Multi-Market, Natively**  
English, German (DACH), French, and Japanese modes â€” with market-specific vocabulary for comp structures, labor law, and contract types. One system, every market.

---

### Section 5 â€” Learning Loop
...
fits you â€” not just what looked good on paper. applyOS isn't
just a tool. It's a system that compounds.
```

**Diagram (monospace, text-based or SVG):**
```
Your profile  â†’  Evaluate a role  â†’  Score + feedback  â†’  Sharper profile
      â†‘____________________________________________________|
```
...
| `Aâ€“F` | Structured scoring rubric |
...
**CTA:** `Get Started â†’` â€” primary blue button, 16px, centered
...
| Tablet 768â€“1280px | 2-col feature grid â†’ 1-col; hero stays centered |
...
- No animations on initial load â€” hero is immediate and static
...
| Fonts | `next/font` â€” Geist + Geist Mono |
...
- No blog, no changelog, no docs â€” those come after v1 launch
